...
void RXWnd::OnTimer(HWND hWnd, UINT ID)
{
...
{
	// 이용한 윈도우 프로퍼티 영역을 제거합니다.
	// 제거해주지 않으면 메모리 릭이 발생합니다.
...
// 윈도우 생성 정보와 윈도우 스타일을 인자로 받습니다.
HRESULT RXWnd::Create(const WndCreateInfo& wndCreateInfo, DWORD dwStyle)
{
	return Create(wndCreateInfo.posInfo.x, wndCreateInfo.posInfo.y,
		wndCreateInfo.resolutionInfo.width, wndCreateInfo.resolutionInfo.height, dwStyle);
}
/*
 * <파일 설명>
 * 가우스-조던 소거법을 이용해서 역행렬을 구해보는 예제입니다.
 * 가우스 소거법을 구현할 줄만 알면 크게 어렵지는 않아요.
 * 전방 소거는 달라진 게 없고 후방 소거라는 과정이 하나 추가되는 것뿐이에요.
 * 후방 소거는 전방 소거에 비해 쉬우니까 걱정하지 않아도 됩니다.
 * 하지만 가우스 소거법을 이해하지 못했다면 가우스-조던 소거법은 엄청 어려워지겠죠 ㄷㄷ
 */
...
public:
	...
	// 가우스-조던 소거법을 이용해서 역행렬을 구합니다.
	// 구하는 방법은 다음과 같습니다.
	//
	// 1. 기존 행렬을 가져오고, 단위행렬을 하나 만듭니다.
	//
	// 2. 기준 행을 이용해서 나머지 행의 기준 요소를 0으로 만들어준다.
	//    한 행 또는 한 열에 어떤 수를 곱해서 더하거나 빼도 행렬식의 값은 변화되지 않습니다.
	//    단위행렬에도 같은 처리를 해줍니다.
	...
	//
	// 5. 후방 소거를 시작합니다.
	//    4행 4열의 값으로 4행을 전부 나눠주고, 단위행렬에도 동일하게 적용합니다.
	//    이때 연산 속도를 위해 기존 행렬에는 나눠줄 필요 없이 1로만 맞춰주면 됩니다.
	//    상삼각행렬이 완성된 이후이므로 0에 숫자를 나눠봤자 의미가 없습니다.
	//
	// 6. 같은 과정을 반복해서 기존 행렬이 단위행렬이 되었다면
	//    단위행렬에서 변화된 행렬을 반환하면 됩니다. 그게 바로 역행렬입니다.
	//
	// GJEM => Gaussian-Jordan Elemination Method
	Matrix4X4 InverseByGJEM()
	{
		float rDet = 1.0f; // 행렬식의 초깃값은 부호를 바꿀 수 있어야 하므로 1로 설정합니다.

		Matrix4X4 mat = *this; // 행렬식을 구하는 게 목적이므로 기존 행렬의 복사본을 이용합니다.
		Matrix4X4 matInverse;  // 단위행렬 -> 역행렬이 될 행렬입니다.
		matInverse.Identity();
		...
		for (int row = 0; row < 4 - 1; ++row)
		{
			...
			if (mat.getElement(row, row) == 0.0f)
			{
				...
				{
					mat.SwapRow(pivot, row);        // 기준 행을 변경합니다, Swap이므로 순서는 무관해요.
					matInverse.SwapRow(pivot, row); // 역행렬로 만들 행렬에도 같은 처리를 해줍니다.
					rDet *= -1.0f;                  // 행렬식의 성질에 의해 행렬식의 부호가 바뀝니다.
				}
				// 중심점을 바꾸지 못했다면 기준 행의 기준 요소는 0이므로 행렬식의 값은 0입니다.
				// 즉, 이 경우는 pivot과 row가 같을 때입니다.
				...
				{
					printf("기준 행의 기준 요소가 0이므로 역행렬이 존재하지 않습니다!\n");
					printf("-----------------------------------\n");
					return matInverse;
				}
			}
			...
			for (int nextRow = row + 1; nextRow < 4; ++nextRow)
			{
				// 이미 기준 요소가 0이라면 계산해줄 필요가 없지만!
				// 이때는 역행렬로 변환해주는 행렬이 따로 존재하므로
				// 이 과정을 생략하면 안 됩니다!!!
				//if (mat.getElement(nextRow, row) == 0.0f)
				//{
				//	continue;
				...
				// nextRow의 모든 요소에 rZeroBase를 적용합니다.
				// 여기서는 열을 이동해야 하므로 col이라는 변수를 따로 사용할게요.
				// 상삼각행렬을 만드는 과정에서 기준 요소의 이전 요소는 이미 0이므로 신경 쓰지 않아도 되지만
				// 위에서 설명한 대로 역행렬로 만들어주는 행렬이 따로 존재하므로 연산 과정을 생략하면 안 됩니다!
				for (int col = 0; col < 4; ++col)
				{
					float rTemp = mat.getElement(nextRow, col) - mat.getElement(row, col) * rZeroBase;
					mat.setElement(nextRow, col, rTemp);

					// 가우스-조던 소거법에서 단위행렬을 역행렬로 만들 때는
					// ZeroBase의 값을 그대로 적용합니다. 절대로 값을 다시 계산하면 안 돼요!
					rTemp = matInverse.getElement(nextRow, col) - matInverse.getElement(row, col) * rZeroBase;
					matInverse.setElement(nextRow, col, rTemp);
				}
			}
		}
		...
		mat.ShowElement();
		printf("-----------------------------------\n");
		printf("행렬식 계산 : %.2f * %.2f * %.2f * %.2f * %.2f = %.2f\n\n",
			rSign, mat.getElement(0, 0), mat.getElement(1, 1),
			mat.getElement(2, 2), mat.getElement(3, 3), rDet);
		...
		// 상삼각행렬을 출력해봅니다.
		printf("matInverse의 현재 상황입니다.\n");
		printf("-----------------------------------\n");
		matInverse.ShowElement();
		printf("-----------------------------------\n");

		// 행렬식이 존재한다면 역행렬을 정의할 수 있습니다.
		// 전방 소거 과정은 끝났으므로 후방 소거만 적용하면 됩니다.
		// 후방 소거는 전방 소거와는 반대로 마지막 행부터 첫 행까지 처리해줘야 합니다.
		for (int row = 4 - 1; row >= 0; --row)
		{
			// 기준 행의 기준 요소를 1로 만들어주기 위해 요소로 나눠줍니다.
			// 역행렬을 구할 행렬에도 똑같이 적용해줍니다.
			// 위에서 설명한 대로 연산 속도를 위해 그냥 1을 써도 되지만
			// 값을 덮어씌우기 전에 먼저 역행렬을 구할 행렬에 나눠줘야 합니다.
			for (int col = 0; col < 4; ++col)
			{
				// 상삼각행렬이 만들어진 후이므로 기준 행의 기준 요소가 0이 될 걱정은 안 해도 됩니다.
				// 애초에 기준 행의 기준 요소가 0이라는 건 위에서 행렬식으로 확인했으니까요.
				float rTemp = matInverse.getElement(row, col) / mat.getElement(row, row);
				matInverse.setElement(row, col, rTemp);
			}

			mat.setElement(row, row, 1.0f); // 기존 행렬을 단위행렬로 만들어주는 과정입니다.
		...
		// 하삼각행렬까지 완성했으면 단위행렬에서 변화된 행렬이 생기는데
		// 그 행렬이 바로 역행렬입니다.
...
int main()
{
	...
	printf("-----------------------------------\n\n");
	Matrix4X4 matInverse = mat.InverseByGJEM();
	...
	printf("-----------------------------------\n");
	matInverse = matInverse.InverseByGJEM();

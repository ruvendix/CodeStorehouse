/*
 * <파일 설명>
 * 가우스 소거법을 이용해서 행렬식을 구해보는 예제입니다.
 * 이번 방법으로 행렬식을 구하기 위해서는 행렬식의 성질을 알아야 해요.
 * 여기에 쓰기에는 내용이 좀 많아서 친절하게 설명된 동영상을 포스팅에 첨부할게요.
 * 그리고 상삼각행렬(Upper Triangle Matrix)과 하삼각행렬(Lower Triangle Matrix)을 알아야 하는데
 * 단위행렬처럼 굉장히 쉬운 개념이므로 이건 쉽게 이해하실 겁니다.
 * 그래도 전체적으로는 꽤 난도가 있으므로 천천히 보셔야 이해될 거예요.
 */
